...
STEPS = ["ask_name", "ask_activity", "ask_target", "ask_actual", "done"]
...
def handle_message(phone, text):
    ...
    if step == "ask_name":
        ...
        send_text(
            phone,
            f"✅ Saved!\nName: {session['data']['name']}\nActivity: {session['data']['activity']}\n"
            f"Target: {session['data']['target']}\nActual: {session['data']['actual']}\n"
            f"Achievement: {achievement}%\n\nType anything to log another entry."
        )
        sessions[phone] = {"step": "ask_name", "data": {}}  # reset for next entry

    else:
        session["step"] = "ask_name"
        send_text(phone, "Hi! What is your name?")
...
def save_to_sheet(phone, data):
    ...
    # Matches sheet headers: Date | Worker Name | Activity | Target | Actual | Achievement%
    sheet.append_row([
